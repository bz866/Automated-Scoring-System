...
def build_hrcnn_model(opts, vocab_size=0, char_vocabsize=0, maxnum=50, maxlen=50, maxcharlen=20, embedd_dim=50, embedding_weights=None, verbose=False, init_mean_value=None):
    ...
    if opts.mode == 'mot':
        ...
        #zcnn = K.reshape(zcnn,(K.int_shape(zcnn)[0],K.int_shape(zcnn)[1],K.int_shape(zcnn)[2],K.int_shape(zcnn)[3]))
        #print(K.int_shape(zcnn)) #(10, 71, 46, 100)
        avg_zcnn = TimeDistributed(Attention(), name='avg_zcnn')(zcnn)
        #print(K.int_shape(avg_zcnn)) #(None, 71,100)
    ...
    #print(K.int_shape(avg_zcnn))
    #avg_zcnn = K.reshape(avg_zcnn,(K.int_shape(avg_zcnn)[1],K.int_shape(avg_zcnn)[2],K.int_shape(avg_zcnn)[3]))
